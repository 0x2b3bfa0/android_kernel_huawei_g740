...
{ 
    ...
    if( light_device_minor == iminor(inode) ){
        APS9900_DBG("%s:light sensor open\n", __func__);
        /*it's not necessary to set this flag everytime*/
        /*aps_first_read = 1;*/
    }
    if( proximity_device_minor == iminor(inode) )
    {
        if((PRXIMITY_OPEN_INIT == get_sensor_prop())&&(PROXIMITY_SUSPEND == atomic_read(&px_status_flag)))
        {
            /*Re-init the register for avoid hardware drop experiment*/
            for ( i=0; i< ARRAY_SIZE(aps9900_init_regdata);i++)
            {
                ret = set_9900_register(this_aps_data, CMD_BYTE|aps9900_init_regdata[i].reg, aps9900_init_regdata[i].data, 0);
                if (ret < 0)
                {
                    printk(KERN_ERR "Ret of init aps9900 regs(%d - %d) is %d\n", aps9900_init_regdata[i].reg, aps9900_init_regdata[i].data, get_9900_register(this_aps_data, aps9900_init_regdata[i].reg, 0));
                    break;
                }
            }
            APS9900_DBG("%s:RE-init register happened\n", __func__);
        }
        ...
        APS9900_DBG("%s:proximity_device_minor == iminor(inode),get wake_lock\n", __func__);
...
static int aps_9900_release(struct inode *inode, struct file *file)
{
    ...
    if( proximity_device_minor == iminor(inode) ){
        APS9900_DBG("%s: proximity_device_minor == iminor(inode),free wake_lock\n", __func__);
        wake_unlock( &proximity_wake_lock);
    }
    if( light_device_minor == iminor(inode) )
        APS9900_DBG("%s: light_device_minor == iminor(inode)\n", __func__);
