...
static void led_kp_set(struct pm8xxx_led_data *led, enum led_brightness value)
{
	...
	/*coverity: Calling function "pm8xxx_mpp_config" without checking return value */
	rc = pm8xxx_mpp_config(led->cdev.gpio, &mpp_config_data);
	if(rc < 0){
...
static u8 val_wled_max_current[WLED_STRING_DEFAULT_NUM] = {0};
#endif
static int
...
#ifdef CONFIG_HUAWEI_KERNEL
	u8 reg_val_264 = 0;
/*move this variable globe*/
	static int wled_old_value = LED_HALF,wled_new_value = LED_HALF;
...
#ifdef CONFIG_HUAWEI_KERNEL
	wled_new_value = value;
#endif
	if (value == 0) {
#ifdef CONFIG_HUAWEI_KERNEL
		/*Record those register value*/
		/*remove this init code to init-wled part and change next to slove coverty*/
		rc = pm8xxx_readb(led->dev->parent,WLED_SYNC_REG, &reg_val_264);
		if (rc)
		{
			dev_err(led->dev->parent, "can't read register WLED_SYNC_REG,line %d"
				" register rc=%d\n", __LINE__, rc);
		}

		/*then write those register to close wled step by step*/
		for(i = 0; i < num_wled_strings; i ++)
		{
			rc = pm8xxx_writeb(led->dev->parent, WLED_MAX_CURR_CFG_REG(i), 0x00);
			if (rc)
			{
				dev_err(led->dev->parent, "can't write register WLED_MAX_CURR_CFG_REG(%d),line %d" 
					" register rc=%d\n", i,__LINE__,rc);
			}
		}
		rc  = pm8xxx_writeb(led->dev->parent, WLED_SYNC_REG, 0x00);
		/*Different string num need different value for SYNC_REG at this step,
		  for 1 string we write 0x0C,for 2 strings we write 0x1E*/
		if(ONE_WLED_STRING == num_wled_strings)
			rc |= pm8xxx_writeb(led->dev->parent, WLED_SYNC_REG, 0x0C);
		else
			rc |= pm8xxx_writeb(led->dev->parent, WLED_SYNC_REG, 0x1E);

		/*WLED_BOOST_CFG_REG should be 0x23 when open and close WLED module,other time be 0x43*/
		rc |= pm8xxx_writeb(led->dev->parent, WLED_BOOST_CFG_REG, 0x23);
		rc |= pm8xxx_writeb(led->dev->parent, WLED_MOD_CTRL_REG, 0x09);
		if (rc) 
		{ 
			dev_err(led->dev->parent, "can't write register WLED_SYNC_REG,WLED_BOOST_CFG_REG,WLED_MOD_CTRL_REG,line %d" 
				" register rc=%d\n", __LINE__, rc);
		}
		mdelay(1);
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		/*ENABLE those register when first open wled*/
		if((0 == wled_old_value) &&(0 != wled_new_value))
		{
			for(i = 0; i < num_wled_strings; i ++)
			{
				rc = pm8xxx_writeb(led->dev->parent, WLED_MAX_CURR_CFG_REG(i), val_wled_max_current[i]);
				if (rc)
				{
					dev_err(led->dev->parent, "can't write register WLED_MAX_CURR_CFG_REG(%d),line %d" 
						" register rc=%d\n", i,__LINE__,rc);
			...
			mdelay(5);
			rc |= pm8xxx_writeb(led->dev->parent, WLED_BOOST_CFG_REG, 0x43);
			if (rc) 
			{ 
				dev_err(led->dev->parent, "can't write register WLED_BOOST_CFG_REG,WLED_BOOST_CFG_REG,WLED_MOD_CTRL_REG,line %d" 
					" register rc=%d\n",__LINE__ ,rc);
				return rc;
			}
		}
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (value == 0) {
		/*When close wled is over,we should wirte those register back*/
		for(i = 0; i < num_wled_strings; i ++)
		{
			rc = pm8xxx_writeb(led->dev->parent, WLED_MAX_CURR_CFG_REG(i), val_wled_max_current[i]);
			if (rc)
		...
		rc |= pm8xxx_writeb(led->dev->parent, WLED_SYNC_REG, reg_val_264);
		rc |= pm8xxx_writeb(led->dev->parent, WLED_BOOST_CFG_REG, 0x43);
		if (rc)
		{
			dev_err(led->dev->parent, "can't write register WLED_SYNC_REG,WLED_BOOST_CFG_REG" 
				" register rc=%d\n", rc);
		}
	}
	wled_old_value = value;
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	/*If need to limit the brightness then do it silently*/
	if ((led->cdev.limit_brightness != -1) && (value > led->cdev.limit_brightness))
		led->cdev.brightness = led->cdev.limit_brightness;
	else
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	u8 wled_whistle_value = 0;

	led->wled_cfg->num_strings = get_wled_string_num();
	wled_whistle_value = wled_whistle_parameter();

	num_wled_strings = led->wled_cfg->num_strings;
	pr_info("%s,WLED string number is %d,wled_whistle_value is 0x%x,config hsad.\n",__func__,num_wled_strings,wled_whistle_value);
#else
	num_wled_strings = led->wled_cfg->num_strings;
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	/*the val equal 0x7F because the LK part initlize a wrong value,
	  so we wipe it and re-init,1 string is 0x13 and 2 strings is 0x37*/
	val = 0;

	if (led->wled_cfg->dig_mod_gen_en)
	{
		if(num_wled_strings == ONE_WLED_STRING)
		{
			val |= 0x10;
		}
		else if(num_wled_strings == TWO_WLED_STRINGS)
		{
			val |= 0x30;
		}
		else 
...
#ifdef CONFIG_HUAWEI_KERNEL
	rc  = pm8xxx_writeb(led->dev->parent, WLED_MOD_CTRL_REG, led->wled_mod_ctrl_val);
	rc |= pm8xxx_writeb(led->dev->parent, WLED_OVP_CFG_REG, 0x55);  /*Qcom patch*/
	rc |= pm8xxx_writeb(led->dev->parent, WLED_BOOST_CFG_REG, 0x43);  /*Qcom patch*/
	if (rc) 
	{
		dev_err(led->dev->parent, "can't write register WLED_MOD_CTRL_REG,WLED_OVP_CFG_REG,WLED_BOOST_CFG_REG,line %d"
			" register rc=%d\n", __LINE__,rc);
	}
	rc = pm8xxx_writeb(led->dev->parent, WLED_CNTRL_15, wled_whistle_value);
	if (rc) 
	{ 
...
static int __devinit pm8xxx_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pcore_data->num_leds; i++) {
		...
		/*read the limited brightness from the product config*/
		limited_brightness = get_led_brightness(curr_led->name, "limit");
		pr_info("%s limited_brightness=%d\n", curr_led->name, limited_brightness);
#else
		led_dat->max_current = led_cfg->max_current;
#endif
		...
		led_dat->cdev.limit_brightness = limited_brightness;
        #endif
