...
struct pm8921_chg_chip {
	...
	int draw_current; /*asked draw_current in function void pm8921_charger_vbus_draw(unsigned int mA)*/
};
...
static int get_prop_mpp5_temp(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("mpp5_temp phy = %lld meas = 0x%llx\n", result.physical,
						result.measurement);
	/* delete shutdown log because the handset will not shutdown because of mpp5 temperature*/
...
static int get_prop_battery_uvolts(struct pm8921_chg_chip *chip)
{
	...
	report_ocv_uv = pm8921_bms_get_ocv_uv();

	pr_debug("ocv_uv = %d", report_ocv_uv);
	
	return report_ocv_uv;
}
#else
static int get_prop_battery_uvolts(struct pm8921_chg_chip *chip)
{
...
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	...
	if (fsm_state == FSM_STATE_ON_CHG_HIGHI_1) {
		if (!pm_chg_get_rt_status(chip, BATT_INSERTED_IRQ)
			...
			/* display not charing when temperature is  too cold or hot */
#ifdef CONFIG_HUAWEI_KERNEL
			|| force_ps_but_not_chg_temp
#endif 
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if((0 < usb_draw_500ma) && (0 < mA) && (USB_WALL_THRESHOLD_MA > mA) && (is_usb_chg_plugged_in(the_chip)))
	{
		pr_err("not set draw lower than 500mA when charger is present!\n");
		return;
	}

	the_chip->draw_current = mA;
...
#define RECHG_FRQ_CONSECUTIVE_COUNT	100
#define RECHG_CONSECUTIVE_COUNT	10
#define DELTA_RECHG_VOLTAGE_MV 20
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	bool ramp = false;
#ifdef CONFIG_HUAWEI_KERNEL
	static int rechg_frq_count = 0,rechg_count = 0;
	bool should_rechg = false;
	unsigned int rechg_voltage = 0;
	u8 chg_cntrl = 0,chg_cntrl_3 = 0;
	struct pm8xxx_adc_chan_result result;

	/*check if current voltage < the rechg voltage every RECHG_FRQ_CONSECUTIVE_COUNT count*/
	rechg_frq_count = ( rechg_frq_count < RECHG_FRQ_CONSECUTIVE_COUNT) ? (rechg_frq_count+1) : 0;
	
	if( RECHG_FRQ_CONSECUTIVE_COUNT == rechg_frq_count )
	{
		/*get if the charging was stopped*/
		rc = pm8xxx_readb(chip->dev->parent, CHG_CNTRL_3, &chg_cntrl_3);
		if (rc) 
		{
			pr_err("Failed to read CHG_CNTRL_3 rc=%d\n", rc);
		}

		rc = pm8xxx_readb(chip->dev->parent, CHG_CNTRL, &chg_cntrl);
		if (rc)
		{
			pr_err("Failed to read CHG_CNTRL rc=%d\n", rc);
		}

		/*get current voltage*/	
		rc = pm8xxx_adc_read(chip->vbat_channel, &result);
		if (rc) {
			pr_err("Failed to read voltage rc=%d\n", rc);
		}
		
		/*get rechg voltage*/	
		rechg_voltage = the_chip->max_voltage_mv - the_chip->resume_voltage_delta;

		if (chip->is_bat_cool)
		{
			rechg_voltage = the_chip->cool_bat_voltage - the_chip->resume_voltage_delta;
		}
		else if (chip->is_bat_warm)
		{
			rechg_voltage = the_chip->warm_bat_voltage - the_chip->resume_voltage_delta;
		}
		
		/*if the charging was not stopped on purpose && the current voltage lower than rechg voltage - DELTA_RECHG_VOLTAGE_MV, should rechg count ++*/	
		if ((!charging_disabled)&&(!force_ps_but_not_chg)&&(!force_ps_but_not_chg_temp)
		     &&((!(chg_cntrl_3 & CHG_EN_BIT)) ||(chg_cntrl & CHG_CHARGE_DIS_BIT))
		     && ((int)result.physical < (rechg_voltage - DELTA_RECHG_VOLTAGE_MV)* UV_PER_MV))
		{
			should_rechg = true;
		}

		rechg_count = should_rechg ? (rechg_count+1) : 0;

		/*restart the charging there if the recharging automatically failed*/
		if( RECHG_CONSECUTIVE_COUNT < rechg_count )
		{
			pm_chg_failed_clear(chip, 1);
			pm_chg_charge_dis(chip, charging_disabled);
			pm_chg_auto_enable(chip, !charging_disabled);
...
{
    int temp = 0, voltage = 0, cur = 0, capacity = 0, draw_current = 0, health = 0, type = 0, status = 0;
    ...
    else
    {
        ...
        draw_current = the_chip->draw_current;/* asked draw current */
        ...
        sprintf(buf, "%-9d  %-9d  %-4d  %-6d  %-6d   %-6d  %-6d  %-6d    ",                                
                    voltage/1000,  (signed short)(cur/1000), capacity, temp/10, draw_current, health, type, status);
        return strlen(buf);
    }
}
...
static void set_charge_data(struct pm8921_chg_chip *chip)
{
    ...
    chip->cool_bat_voltage =  charger_individual_data->max_voltage;
...
/*============================================================================================
FUNCTION        set_usb_draw_500ma

DESCRIPTION     Internal function to usb_draw_500ma.
...
=============================================================================================*/
static int set_usb_draw_500ma(const char *val, struct kernel_param *kp)
{
	int ret;
...
module_param_call(usb_draw_500ma, set_usb_draw_500ma, param_get_uint,
					&usb_draw_500ma, 0644);
#endif
