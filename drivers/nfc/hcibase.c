...
void DumpBuffer(char *buffer, int len, int direction)
{
    ...
    /* for warning clear of Coverity (NFC) */
	if(direction == 0)
	{
		snprintf(finalStr, sizeof(finalStr), "PC -> IFD: %s\n\n", ascBuf);
	}
	else if(direction == 1)
	{
		snprintf(finalStr, sizeof(finalStr), "IFD -> PC: %s\n\n", ascBuf);
	}
	else 
	{
		snprintf(finalStr, sizeof(finalStr), "DumpBuffer: %s\n\n", ascBuf);
	}
	...
	while(1)
	{
		...
		if(condBranchNeededFlag == 1)
		{
			if(tmpCmdLineElem[idx].type == CMD_TYPE_CONDITIONAL)
			{
				...
				{
					strncpy((char *)scriptReqBuf, tmpCmdLineElem[idx].scriptCmdLine, sizeof(scriptReqBuf)-1);
		...
		snprintf(tmpBuf, sizeof(tmpBuf), "idx: %d, ret: %d\n", idx - 1, ret);
