...
#define SUMSUNG_DDR  	"SUMSUNG_DDR"
#define ELPIDA_DDR  	"ELPIDA_DDR"
#define HYNIX_DDR  	    "HYNIX_DDR"
...
typedef struct
{
    u32 battery_status;    /* battery connect status */
    unsigned char sb_seme_data[16];
	//sbl3 version
    char  sbl3_version[64];
    u32 lpddr_info;    
...
char * get_ddr_name(void)
{
    unsigned int ddr_factory_id = 0;
    char * ddr_info = "UNKNOW_DDR";
    ...
    ddr_factory_id = param->lpddr_info;
    ddr_factory_id &= 0xff;     

	 switch (ddr_factory_id)
	 {
		case SUMSUNG_ID:
...
/* BU5D10365 liujinggang 20100521 end > */
	...
	ddr_list_name = get_ddr_name();
	if (NULL == ddr_list_name)
	{
		ddr_list_name = "FAIL";
	}
    ...
    "board_name:\n%s\n"
    "ddr_manufactory:\n%s\n",
	lcd_name,sensors_list_name,compass_gs_name,battery_name,touch_info,g_current_board_id,board_name,ddr_list_name);
