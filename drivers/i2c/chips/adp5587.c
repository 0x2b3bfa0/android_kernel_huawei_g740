...
{
	/*coverity: without checking return value */
	int ret = 0;
	...
	while (key_event_counter == 0)
	{
		...
		/*coverity: without checking return value */
		timeout = schedule_timeout(msecs_to_jiffies(20));
		if(!timeout)
		{
			printk(KERN_ERR "%s: Wait for 20ms schedule timeout! and Try doing msleep again!\n", __func__);
			msleep(20);
		}
...
exit_clear_interrupt_and_enable_irq:
	//Clear interrupt
	/*coverity: without checking return value */
	ret = i2c_smbus_write_byte_data(adp5587_data->client,INT_STAT,KE_IEN);
	if(ret)
	{
		printk(KERN_ERR "adp5587_work_fuc: Fail to clear INT_STAT flag, error=%d!\n", ret);
	}
	KEYPAD_DEBUG("%s:--- \n",__FUNCTION__);
...
exit_clear_interrupt_and_enable_irq:
	...
	/*coverity: without checking return value */
	ret = i2c_smbus_write_byte_data(client,INT_STAT,KE_IEN);
	if(ret)
	{
		printk(KERN_ERR "%s: Fail to clear INT_STAT flag, error=%d\n",
				__FUNCTION__, ret);
	}
	return 0;
}
