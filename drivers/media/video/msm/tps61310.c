...
#define FLASH_REFRESHED_TIME 8 /*8s*/
static struct hrtimer flash_timer;
static struct work_struct flash_work;
static atomic_t timer_is_run = ATOMIC_INIT(0);
...
int tps61310_set_flash(unsigned led_state)
{
    ...
    if(atomic_read(&timer_is_run))
    {
        hrtimer_cancel(&flash_timer);
        atomic_dec(&timer_is_run);
    }
    switch (led_state)
    {
    ...
    case MSM_CAMERA_LED_MMI:
        ...
        tps61310_i2c_write( tps61310_client,0x01, 0x40 );
        tps61310_i2c_write( tps61310_client,0x02, 0x40 );
        /* move to second interface */
        break;

        /* add second interface for MMI */
        case MSM_CAMERA_LED_MMI_SECOND:
        {
            printk("MSM_CAMERA_LED_MMI_SECOND\n");
            tps61310_i2c_write( tps61310_client,0x00, 0x02 );/*switch to the second led*/
            tps61310_i2c_write( tps61310_client,0x05, 0x6F );
            tps61310_i2c_write( tps61310_client,0x01, 0x40 );
            tps61310_i2c_write( tps61310_client,0x02, 0x40 );
        }
