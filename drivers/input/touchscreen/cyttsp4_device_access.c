...
#include <linux/cyttsp4_device_access.h>
...
static int _cyttsp4_get_ebid_row_size(struct device *dev)
{
	...
	u8 cmd_buf[CY_CMD_CAT_GET_CFG_ROW_SZ_CMD_SZ] = {0};
	u8 return_buf[CY_CMD_CAT_GET_CFG_ROW_SZ_RET_SZ] = {0};
	int rc;
...
struct kobject* tp_get_touch_screen_obj(void)
{
	if( NULL == touch_screen_kobject_ts )
	{
		...
		{
			pr_err("%s: create touch_screen kobjetct error!\n", __func__);
			return NULL;
		}
		else
		{
			pr_info("%s: create sys/touch_screen successful!\n", __func__);
		}
	}
	else
	{
		pr_info("%s: sys/touch_screen already exist!\n", __func__);
	}
	
	return touch_screen_kobject_ts;
}
struct cypress4_tp_cap_info_t capacitance_data;
...
/*MAX_CAPACITANCE_LEN should bigger than 7 * tx *rx*/
static	int  g_capacitance_count = 0;
static char *g_touch_capacitance = NULL;

/* save each value of capacitance to buf */
#undef TP_TMP_BUF_SIZE
#define TP_TMP_BUF_SIZE 7
static void record_tp_capacitance(enum check_data_type type, int value)
{
	char buf[TP_TMP_BUF_SIZE] = {0};
	snprintf(buf, TP_TMP_BUF_SIZE, "%d\t", value);
	/*The strncat's third parameter should be the remain size of first parameter,
	  but we use a point,so just add at tail*/
	strncat(g_touch_capacitance, buf, strlen(buf));
	g_capacitance_count++;
	if(0 == g_capacitance_count % capacitance_data.tx_lines)
	{
...
static int out_of_range(enum check_data_type type, int value, int index)
{
	...
	{
		case CY_CHK_MUT_RAW:    
			if(value < capacitance_data.min_mut_cap || value > capacitance_data.max_mut_cap) // mutual capacitance rang for raymond
			{
				printk("Mutual-Capacitance %s!\n",__func__);
				return 1;
			}
			break;

		case CY_CHK_SELF_RAW:
			if(value < capacitance_data.min_self_cap || value > capacitance_data.max_self_cap)  // self capacitance rang for raymond
			{
				printk("Self-Capacitance %s!\n",__func__);
				return 1;
			}
			break;
...
static int cyttsp4_get_data_and_check(struct device* dev, retrieve_func ret_func, enum check_data_type type, int offset)
{
	...
	int read_byte = CY_CMD_CAT_RET_PANEL_DATA_RET_SZ + cmd_param_ofs;
	...
	dev_info(dev, "%s: _cyttsp4_ret_scan_data_cmd(): num_element:%d\n",
	...
	{
		dev_info(dev, "%s, read_byte > CY_MAX_READ_DATA=%d\n", __func__, CY_MAX_READ_DATA);
		...
		left_over_element = dad->heatmap.numElement - CY_MAX_SENSOR_READ_DATA;
	...
	dad->ic_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_L + cmd_param_ofs] =
		LOW_BYTE(read_element_offset);

	dev_info(dev, "%s: data_idx=%d\n", __func__,data_idx);

	 /* check capacitor range */
	rc = cyttsp4_check_range(type, endian, datasize, dad, data_idx);
	if(rc < 0)
	{
		dev_info(dev, "%s cyttsp4_check_range failed!!!\n", __func__);
		return -1;
	}
...
static int cyttsp4_check_raw_data(struct device *dev)
{
	...
	/* Start scan  only scan one time is enough*/
	rc = _cyttsp4_exec_scan_cmd(dev);
	if (rc < 0)
	{
		printk("%s,Scan failed\n",__func__);
		return 0;
	}

	for(i = 0; i < 2; ++i)
	{
		/*All parameters should be self-adapt*/
		if(0 == i)
		{
			type = CY_CHK_MUT_RAW;
			dad->heatmap.numElement = (capacitance_data.tx_lines*capacitance_data.rx_lines); 
			dad->heatmap.dataType = CY_MUT_RAW;
		}
		else
		{
			type = CY_CHK_SELF_RAW;
			dad->heatmap.numElement = capacitance_data.tx_lines;//rx_line's self cap is not useful
			dad->heatmap.dataType = CY_SELF_RAW;
		}
		dev_info(dev, "%s: type is:%d\n", __func__, type);

		/*move _cyttsp4_exec_scan_cmd out of for circle*/
		/* retrieve scan data */
...
//make the "FAIL" "PASS" string as same as 8x25Q
...
int  cyttsp4_get_panel_data_check(char **buf)
{
	...
	/*don't release here,call cyttsp4_release_exclusive after short detect*/
	...
	dev_err(dev, "%s:begin to cyttsp4_check_short_data check\n",__func__);
...
cyttsp4_check_short_data_err_release:
	if(need_output){
		...
		cyttsp4_out_to_buf(rc, buf);
		/* release here,same as last err_release*/
		rc= cyttsp4_release_exclusive(dad->ttsp);
		if (rc< 0) 
		{
			rc = 0;//use this to justify what to output to the user-space buf
...
static void cyttsp4_fw_calibrate(struct cyttsp4_device *ttsp)
{
	...
	pm_runtime_get_sync(dev);
	dev_err(dev, "%s:begin to cyttsp4_fw_calibrate\n",__func__);
...
/*caller will release the memory allocated here*/
static int _cyttsp4_get_ignore_list(int **data, int *count, char *name)
{
	int *p = NULL;
	int i = 0;

	if (NULL == data || NULL == count || NULL == name)
	{
		pr_err("%s Invalid parameter!\n",__func__);
		return -1;
	}

	/* allocate the memory, this memory is released by caller even the
	   function returns failure*/
	p = kzalloc(sizeof(int) * (*count), GFP_KERNEL);
	if (NULL == p)
	{
		pr_err("%s: memory allocation failed for %s ignore list\n", __func__, name);
		*count = 0;
		cyttsp4_out_to_buf(0, &touch_mmi_test_result);
		return -1;
	}

	*data = p;
	/*get the list from the product config*/
	get_tp_capacitance_ignore_list(p, name, count);
	if (!capacitance_data.ignore_list_size_self_cap)
	{
		pr_err("%s failed to get %s ignore list\n", __func__, name);
		return -1;
	}
	...
	for (i = 0; i < *count; ++i)
	{
		pr_info("tp cap info ignore list for %s [%d]: %d\n", name, i ,p[i]);
	}

	return 0;
}

static void _cyttsp4_release_ignore_list(void)
{
	/*check and release memory allocated for ignore list*/
	if (capacitance_data.ignore_list_self_cap)
	{
		kfree(capacitance_data.ignore_list_self_cap);
		capacitance_data.ignore_list_self_cap = NULL;
	}

	if (capacitance_data.ignore_list_mut_cap)
	{
		kfree(capacitance_data.ignore_list_mut_cap);
		capacitance_data.ignore_list_mut_cap = NULL;
	}
	return;
}
	...
	struct kobj_attribute *attr,char *buf)				
{
	...
	struct cypress4_tp_cap_info_t *cap_info = NULL;
	...
	get_tp_capacitance_info(&cap_info);

	/* check if capacitance data is ok */
	if( false == cap_info->data_ok )
	{
		pr_err("%s: the capacitance infomation is not proper!\n", __func__);
		return -EINVAL;
	}
	capacitance_data = *cap_info;
	pr_info("tp cap info min_mut_cap    : %d\n", capacitance_data.min_mut_cap );
	pr_info("tp cap info max_mut_cap    : %d\n", capacitance_data.max_mut_cap );
	pr_info("tp cap info min_self_cap   : %d\n", capacitance_data.min_self_cap );
	pr_info("tp cap info max_self_cap   : %d\n", capacitance_data.max_self_cap );
	pr_info("tp cap info tx_lines       : %d\n", capacitance_data.tx_lines );
	pr_info("tp cap info rx_lines       : %d\n", capacitance_data.rx_lines );
	pr_info("tp cap info ignore_list_size_self_cap : %d\n", capacitance_data.ignore_list_size_self_cap );
	pr_info("tp cap info ignore_list_size_mut_cap  : %d\n", capacitance_data.ignore_list_size_mut_cap );
	pr_info("tp cap info data_start_byte           : %d\n", capacitance_data.data_start_byte );

	/*7 is one capacitance number's max length,All parameters should be self-adapt*/
	if(NULL == g_touch_capacitance)
	{
		g_touch_capacitance = kzalloc(7 * sizeof(char) * capacitance_data.tx_lines * capacitance_data.rx_lines, GFP_KERNEL);
		if(NULL == g_touch_capacitance)
		{
	...
	memset(g_touch_capacitance, 0, 7 * capacitance_data.tx_lines * capacitance_data.rx_lines);

	if (capacitance_data.ignore_list_size_self_cap > 0)
	{
		if (_cyttsp4_get_ignore_list(&capacitance_data.ignore_list_self_cap, 
			&capacitance_data.ignore_list_size_self_cap, 
			"self"))
			goto exit;
	}

	if (capacitance_data.ignore_list_size_mut_cap > 0)
	{
		if (_cyttsp4_get_ignore_list(&capacitance_data.ignore_list_mut_cap, 
			&capacitance_data.ignore_list_size_mut_cap, 
			"mut"))
			goto exit;
	}
...
exit:
	if(0==strcmp(touch_mmi_test_result,"Fail")){
		printk("%s: cyttsp4_get_panel_data_check Fail,calibrate and attempt to test again\n", __func__);
	...
	pr_info("touch_mmi_test_result : %d\n", rc);
	pr_info("touch_mmi_test_result : %s\n", touch_mmi_test_result);
