...
int alarm_set_rtc_alarm(long time_sec, bool enable_irq)
{
	...
	getnstimeofday(&tmp_time);
	err = rtc_read_time(alarm_rtc_dev, &rtc_current_rtc_time);
	if (err < 0) {
		pr_alarm(ERROR, "alarm_set_rtc_alarm: Failed to read time\n");
	}
...
static int alarm_resume(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		pr_alarm(ERROR, "alarm_resume: Failed to read time\n");
	}
	rtc_tm_to_time(&rtc_current_rtc_time, &rtc_current_time);
