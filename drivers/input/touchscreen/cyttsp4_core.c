...
/*TP Glove func should be close at first time*/
...
static int cyttsp4_core_wake_(struct cyttsp4_core_data *cd)
{
	...
	if (rc < 0) {
		...
		rc = cyttsp4_adap_read(cd->core->adap, CY_REG_BASE, &mode,
				sizeof(mode));
		if(rc < 0)
			dev_err(dev, "%s: cyttsp4_adap_read fails r=%d\n",
				__func__, rc);
	} else
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
	...
	rc = set_mode(cd, CY_MODE_OPERATIONAL);
	if(rc < 0)
		pr_err("%s,set TP operation mode failed\n",__func__);
...
static int add_sensitivity_sysfs_interfaces(struct device *dev)
{
	...
	error |= sysfs_create_link(kobject_ts,&dev->kobj,"glove_func");
	if(error)
		goto undo_glove;
	return 0;
undo:
	...
	return -ENODEV;
undo_glove:
	sysfs_remove_link(&dev->kobj, "glove_func");
	dev_err(dev, "%s: failed to create glove sysfs_link ,error = %d\n", __func__,error);
	return 0;
}
/*delete useless code*/
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	mutex_lock(&cd->system_lock);
	cd->startup_state = STARTUP_ILLEGAL;
	mutex_unlock(&cd->system_lock);
	dev_dbg(dev, "%s: schedule startup\n", __func__);
	...
	/*delete the codes which cannot reach*/
