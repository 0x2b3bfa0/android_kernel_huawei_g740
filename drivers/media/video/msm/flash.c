...
static atomic_t msm_camera_led_flag = ATOMIC_INIT(0);

int msm_flash_get_stored_led_state(void)
{
	int led_state= atomic_read(&msm_camera_led_flag);
...
int call_led_set_state(unsigned led_state)
{
	...
	if  ( NULL != flash_probed_succeed.set_led_state )
	{
		...
		CDBG("%s : LED state is %d.\n", __func__, led_state);
		/* update the state only if the set state is successful*/
		if (!ret)
		{
			atomic_set(&msm_camera_led_flag, led_state);
		}
	}
	else
	{
