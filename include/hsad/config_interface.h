...
#define RF_DEFAULT	"UnSupport"
#define MAX_RF_NAME_LEN	40
...
typedef struct _tp_test_data{
    const int min_mut_cap;
    const int max_mut_cap;
    const int mut_tolerance;
    const int min_self_cap;
    const int max_self_cap;
    const int self_tolerance;
    const int tx_lines;
    const int rx_lines;
    const int first_reg_num;
    const int ignore_reg_count;
}tp_test_data;
/* tp capacitance infomation */
typedef struct cypress4_tp_cap_info_t{
    int min_mut_cap;
    int max_mut_cap;
    int min_self_cap;
    int max_self_cap;
    ...
    int data_start_byte;
    int *ignore_list_self_cap;
    int *ignore_list_mut_cap;
}cypress4_tp_cap_info;

extern void get_tp_capacitance_ignore_list(int* data, char* name, int* count);
...
typedef enum
{
	BY_WLED = 0,//use wled to control backlight
	BY_IC ,//use TI IC to control backlight
	CTRL_BL_UNKNOW = 0x7FFFFFFF,
}hw_lcd_ctrl_bl_dev;
#define WLED_STRING_DEFAULT_NUM 3
#define WLED_WHISTLE_DEFAULT_VALUE 0xDF
...
#endif
extern void get_rf_adapt(char *pname,int length);
...
extern hw_sensor_prop get_sensor_prop(void);
...
extern hw_lcd_ctrl_bl_dev  get_lcd_ctrl_bl_dev(void);
...
extern u8 get_wled_string_num(void);
extern u8 wled_whistle_parameter(void);
