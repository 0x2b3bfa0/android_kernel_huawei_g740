...
static uint8_t digital_gain_for_awb_r = 0;
static uint8_t digital_gain_for_awb_g = 0;
static uint8_t digital_gain_for_awb_b = 0;
static enum sensor_module_type imx134_type = MODULE_SUNNY;
...
static struct msm_camera_i2c_reg_conf imx134_start_settings[] = {
	{0x0100, 0x00}, //to stop stream again forcibly
	{0x3A43, 0x01}, //to clear mode (enter standby)
	{0x0100, 0x01},
};
...
static struct msm_camera_i2c_reg_conf imx134_recommend_settings_standard[] = {
	//Basic Config
	{0x0101, 0x00},
	{0x0105, 0x01},
	{0x0110, 0x00},
...
static struct msm_camera_i2c_conf_array imx134_init_conf_standard[] = {
    {&imx134_recommend_settings_standard[0],
    ARRAY_SIZE(imx134_recommend_settings_standard), 0, MSM_CAMERA_I2C_BYTE_DATA},
};
...
#define  MID1       0x83
#define  MID2       0x75
//value below need to be modified according to the golden samples
#define R_OVER_G_SUNNY         0x0244    //#10
#define B_OVER_G_SUNNY         0x0240
#define R_OVER_G_LITEON        0x0240    //#729
#define B_OVER_G_LITEON        0x0250

uint16_t data_checksum = 0;
uint16_t module_id_checksum = 0;
uint16_t global_checksum = 0; 
uint8_t module_id[5]={0}; 
static uint8_t otp_data[286]={0};
...
int32_t  imx134_write_awb_to_sensor(uint16_t* awb_data) 
{
	...
	uint16_t r_over_g_base     =   0;
	uint16_t b_over_g_base    =   0;
	...
	if(MODULE_SUNNY == imx134_type)
	{
		r_over_g_base = R_OVER_G_SUNNY;
		b_over_g_base = B_OVER_G_SUNNY;
	}
	else if (MODULE_LITEON == imx134_type)
	{
		r_over_g_base = R_OVER_G_LITEON;
		b_over_g_base = B_OVER_G_LITEON;
	}
	...
	imx134_write_sensor(0x020E,0x01);
	imx134_write_sensor(0x020F,g_test_l8);
	imx134_write_sensor(0x0210,0x01);
	imx134_write_sensor(0x0211,r_test_l8);
	imx134_write_sensor(0x0212,0x01);
	imx134_write_sensor(0x0213,b_test_l8);
	imx134_write_sensor(0x0214,0x01);
	imx134_write_sensor(0x0215,g_test_l8);  

	digital_gain_for_awb_r = r_test & 0xFF;
	digital_gain_for_awb_g = g_test & 0xFF;
	digital_gain_for_awb_b = b_test & 0xFF;
...
int32_t imx134_read_module_id_from_eeprom(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	...
	if( 0x1 == (module_id[4] >> 4)){		
		...
		if (0 == strcmp(board_name, "G718"))
			data->sensor_name="imx134_sunny_g718";
		else
			data->sensor_name="imx134_sunny";
		goto END; 
	}
	...
	if( 0x3 == (module_id[4] >> 4)){		
		...
		if (0 == strcmp(board_name, "g527"))
			data->sensor_name="imx134_g527";
		imx134_type = MODULE_LITEON;
		data->actuator_info->cam_name = MSM_ACTUATOR_MAIN_CAM_7;
	}
END:
	for(i = 0; i < sum; i++) 
	{ 
...
int32_t imx134_match_module(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = imx134_read_module_id_from_eeprom(s_ctrl);
	if (rc < 0){ 
		return rc; 
	}
...
/* for project G718, hw guy use DC to provide 1.05v, so we use 1.2v to control the DC. */
/* This is just for G718 */
static struct camera_vreg_t msm_imx134_cam_vreg_cust[] = {
	{"cam_avdd", REG_LDO, 2850000, 2850000, 85600,0},
	{"cam_iovdd", REG_VS, 0, 0, 0,0},
	{"cam_dvdd", REG_LDO, 1200000, 1200000,200000,2000},
};
void imx134_customize_sensor_config(struct msm_sensor_ctrl_t * s_ctrl)
{
    char board_name[MAX_SENSOR_NAME] = {0};
    ...
    if(s_ctrl && s_ctrl->msm_sensor_reg)
    {
        ...
        {
            s_ctrl->msm_sensor_reg->init_settings = imx134_init_conf_standard;
            /* here we can confirm that the pointer could NOT be NULL, ignore the NULL checking here dedicatedly. */
            s_ctrl->sensordata->sensor_platform_info->cam_vreg = msm_imx134_cam_vreg_cust;
        }   
        else
        {
            s_ctrl->msm_sensor_reg->init_settings = imx134_init_conf;
        }
    }
}
		...
		uint16_t gain, uint32_t line)
{
	uint32_t fl_lines;
	uint8_t offset;
	uint32_t gain_dig_dec_r = digital_gain_for_awb_r,
			gain_dig_dec_g = digital_gain_for_awb_g,
			gain_dig_dec_b = digital_gain_for_awb_b;
	int8_t dig_gain_to_write = false;
	...
	if((MSM_SENSOR_RES_QTR != s_ctrl->curr_res) && gain > 0xE0)
	{
		gain_dig_dec_r = (256 + digital_gain_for_awb_r) * 32 / (256 - gain) -256;
		if(gain_dig_dec_r > 0xFF)
			gain_dig_dec_r = 0xFF;
		
		gain_dig_dec_b = (256 + digital_gain_for_awb_b) * 32 / (256 - gain) -256;
		if(gain_dig_dec_b > 0xFF)
			gain_dig_dec_b = 0xFF;
		
		gain_dig_dec_g = (256 + digital_gain_for_awb_g) * 32 / (256 - gain) -256;
		if(gain_dig_dec_g > 0xFF)
			gain_dig_dec_g = 0xFF;
		
		gain = 0xE0;
		dig_gain_to_write = true;

		CDBG("%s: dig_r/g/b=%x / %x / %x \n",
			__func__,gain_dig_dec_r, gain_dig_dec_g, gain_dig_dec_b);
	}
	...
	if(true == dig_gain_to_write)
	{
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
					0x020F, gain_dig_dec_g, MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
					0x0211, gain_dig_dec_r, MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
					0x0213, gain_dig_dec_b, MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
					0x0215, gain_dig_dec_g, MSM_CAMERA_I2C_BYTE_DATA);
	}
...
static struct msm_sensor_fn_t imx134_func_tbl = {
	...
	.sensor_customize_sensor_config = imx134_customize_sensor_config,
};
