		...
		u32 fw_ver_new, u32 fw_revctrl_new_h, u32 fw_revctrl_new_l)
{
	...
	pr_info("%s: img vers:0x%04X new vers:0x%04X\n", __func__,fw_ver_img, fw_ver_new);
	...
	pr_info("%s: img revctrl_h:0x%04X new revctrl_h:0x%04X\n",
	...
	pr_info("%s: img revctrl_l:0x%04X new revctrl_l:0x%04X\n",
...
static int cyttsp4_fw_calibration_attention(struct cyttsp4_device *ttsp)
{
	...
	dev_vdbg(dev, "%s\n", __func__);
	/*Free the Semaphore,to notice fw upgrade finish*/
		...
		const u8 *fw_img, int fw_size)
{
	...
	if (rc < 0) {
		printk("%s: Firmware update failed with error code %d\n",
			__func__, rc);
	} else if (data->loader_pdata &&
			(data->loader_pdata->flags & CY_FLAG_AUTO_CALIBRATE)) {
		/* set up startup call back */
		pr_info("%s: Adding callback for calibration\n",__func__);
		INIT_COMPLETION(data->sysinfo_update);
		rc = cyttsp4_subscribe_attention(ttsp, CY_ATTEN_STARTUP,
				cyttsp4_fw_calibration_attention, 0);
		if (rc) {
			printk("%s: Failed adding callback for calibration\n",__func__);
			printk("%s: No calibration will be performed\n",__func__);
			rc = 0;
		}
	} else { 
		INIT_COMPLETION(data->sysinfo_update);
	...
	if(rc < 0){
		printk("%s: Failed adding release exclusive and request restart\n",__func__);
		rc = 0;
	}
exit:
	pm_runtime_put(dev);
		...
		const struct firmware *fw)
{
	...
	/*For COB always update if not same, for COF update when its newer*/
	if (TP_COF == get_touch_type())
	{
		if (1 == upgrade)
			return 1;
	}
		...
		void *context)
{
	...
	if (upgrade) {
		pr_info("%s: Found firmware,FW upgrade begin\n", __func__);
		...
		printk("%s,Firmware upgrade is not necessary,FW Version is same or latest\n",__func__);
...
static int upgrade_firmware_from_builtin(struct cyttsp4_device *ttsp)
{
	...
	dev_vdbg(dev, "%s: Enabling firmware class loader built-in\n",
		__func__);
	/*firmware name self adapt by board_id*/
	...
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			fw_file_name, dev, GFP_KERNEL, ttsp,
...
/*return the tp firmware upgrade status*/
static int recovery_fw_update_status;
#ifdef CONFIG_TOUCHSCREEN_HUAWEI_CYTTSP4_RECOVERY_FW_UPDATE
static void _cyttsp4_firmware_cont_builtin_recovery(const struct firmware *fw,void *context)
{
	struct cyttsp4_device *ttsp = context;
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	...
	/*Update without firmware version check*/
	_cyttsp4_firmware_cont(fw, ttsp);
	recovery_fw_update_status = 0;
	data->builtin_bin_fw_status = 0;
	complete(&data->builtin_bin_fw_complete);
	return;
...
static int upgrade_firmware_from_builtin_recovery(struct cyttsp4_device *ttsp)
{
	...
	/* wait until FW binary upgrade finishes */
	wait_for_completion(&data->builtin_bin_fw_complete);
	return recovery_fw_update_status;
}
#endif
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	{
		/*Do force calibration when the config upgrade finish*/
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	pr_info("%s: img_crc:0x%04X new_crc:0x%04X\n",
		__func__, data->si->ttconfig.crc, cfg_crc_new);

	if (cfg_crc_new != data->si->ttconfig.crc) {
		printk("%s: Config CRC different, will upgrade\n",__func__);
		return 1;
	}

	pr_info("%s Config CRC equal, will NOT upgrade\n", __func__);
	return 0;
...
static int upgrade_ttconfig_from_platform(struct cyttsp4_device *ttsp)
{
	...
	pr_info("Fw config upgrade Enter....\n");
	...
	if (data->si == NULL)
	{
		printk("%s: Fail get sysinfo pointer from core,Maybe FW is missing or damage\n",__func__);
		/*let the TP sleep if the system info is not available*/
		pm_runtime_put(dev);
		return rc;
	}
	pm_runtime_put(dev);
	/*FW config should match module id and product family*/
	...
	{
		if ((pannel_id == param_map->id) && (0 == strcmp(product_family,param_map->product_family)))
		{
			param_regs = param_map->param;
			printk("%s: pannel_id = %d, param_map->id = %d,Config find successful.\n",__func__,pannel_id,param_map->id);
			break;
		}
	}
	if (NULL == param_regs)
...
static char * get_touch_module_name(u8 module_id)
{
	switch(module_id)
	{
		...
		case JUNDA:
			return "JUNDA";
		case 6:
			return "LENSONE";//ofilm's id = 6 will not support
		case 7:
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	data->si = cyttsp4_request_sysinfo(ttsp);
	if (data->si == NULL)
		dev_err(dev, "%s: Fail get sysinfo pointer from core,FW should be upgrade\n",
			__func__);
	pm_runtime_put(dev);
	...
	/*Allow COF to update newer firmware and config miss-match*/
	...
	if (!upgrade_firmware_from_builtin(ttsp))
	{
		if(!wait_for_completion_timeout(&data->sysinfo_update,msecs_to_jiffies(5000)))
			...
			pr_info("%s sysinfo is ok,FW upgrade done\n",__func__);  
	}
#endif

#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_PLATFORM_TTCONFIG_UPGRADE
	/*upgrade FW config by .h file,just for COB*/
	...
	snprintf(touch_info, sizeof(touch_info), "CYPRESS_TMA463_%s.%d",module_name,ttconfig_version);
	return;
}
#ifdef CONFIG_TOUCHSCREEN_HUAWEI_CYTTSP4_RECOVERY_FW_UPDATE
static void cyttsp4_fw_recovery_upgrade(struct cyttsp4_loader_data *data)
{
	...
	if (!upgrade_firmware_from_builtin_recovery(ttsp))
		dev_info(dev,"%s upgrade_firmware_from_builtin_recovery finished \n",__func__ );
#endif

	data->si = cyttsp4_request_sysinfo(ttsp);
	if (data->si == NULL)
	{
		dev_err(dev, "%s: Fail get sysinfo pointer from core,FW upgrade maybe failed\n",__func__);
		return ;
	}
	pannel_id = data->si->si_ptrs.pcfg->panel_info0;
	ttconfig_version = data->si->ttconfig.version & CY_VER_MASK;
...
static ssize_t update_firmware_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if(data == NULL)
	{
		printk("%s,data is null,manual FW upgrade failed\n",__func__);
		return -EFAULT ;
	}
	if( '2' == buf[0] )
...
static int ts_firmware_file(void)
{
	...
	if (!kobject_ts)
	{
		printk("%s,Cypress4 create kobjetct error!\n",__func__);
		return -1;
	}
...
static int cyttsp4_loader_probe(struct cyttsp4_device *ttsp)
{
	...
	rc = ts_firmware_file();
	if(rc)
		printk("%s,Cypress creat FW_Update failed\n",__func__);
#endif
