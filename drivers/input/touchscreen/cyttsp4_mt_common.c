...
static int cyttsp4_mt_attention(struct cyttsp4_device *ttsp)
{
	...
	{
		printk("%s: Ignoring report while suspended\n",
			__func__);
	}
...
static void cyttsp4_mt_close(struct input_dev *input)
{
	...
	dev_dbg(dev, "%s\n", __func__);
	/*note:It will be not necessary to lift when power down*/
	/*cyttsp4_lift_all(md),reset will do this job*/
...
static void cyttsp4_mt_early_suspend(struct early_suspend *h)
{
	...
	/*do lift at suspend ,not early suspend;
	otherwise,pm_runtime_put will make suspend be called directly*/
	pm_runtime_put(dev);
...
static void cyttsp4_mt_late_resume(struct early_suspend *h)
{
	...
	/*We should set the suspend status at resume,not late resume
	otherwise,pm_runtime_get will make resume be called directly*/
}
...
/*suspend flag should set at that two function*/
static int cyttsp4_mt_suspend(struct device *dev)
{
	struct cyttsp4_mt_data *md = dev_get_drvdata(dev);

	dev_dbg(dev, "%s\n", __func__);
	/*mutex to make lift func run uniqueness*/
	mutex_lock(&md->report_lock);
