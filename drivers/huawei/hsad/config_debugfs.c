...
static int add_dentry_to_list(struct dentry *pdentry, char* path)
{
	...
	if (pNode)
	{
        ...
        strncpy(pNode->path, path, sizeof(pNode->path)-1);
        pNode->path[sizeof(pNode->path)-1] = '\0';
...
static int generate_common_debugfs_line(struct dentry *root, const char* key)
{
	...
	while(b)
	{
		...
		if (b)
		{
            ...
            strncat(fullpath, "/", DEBUGFS_PATH_MAX_LENGTH-strlen(fullpath)-1);
            strncat(fullpath, name, DEBUGFS_PATH_MAX_LENGTH-strlen(fullpath)-1);
            fullpath[DEBUGFS_PATH_MAX_LENGTH-1] = '\0';
...
static ssize_t config_boardid_read(struct file *filp,  char  __user *buffer,size_t count, loff_t *ppos)
{
    ...
    len = snprintf(idarray, sizeof(idarray)-1, "board id: 0x%02x\n", g_current_board_id);
...
static ssize_t config_common_read(struct file *filp, char __user *buffer,size_t count, loff_t *ppos)
{
	...
	while(NULL != pconfig->key )
	{
        if ((NULL != pkey) && (0 == strcmp(pkey, pconfig->key)))
		{
...
int  config_debugfs_init(void)
{
    ...
    debugfs_create_file("id", 0444, boardid_debugfs_root, NULL, &config_boardid_fops);
