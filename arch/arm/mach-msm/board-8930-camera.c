...
static struct gpiomux_setting cam_settings[] = {
	...
	{
		.func = GPIOMUX_FUNC_GPIO, /*reset suspend*/
		.drv = GPIOMUX_DRV_2MA,
		.pull = GPIOMUX_PULL_DOWN,
		.dir = GPIOMUX_OUT_LOW,
	},
...
static struct msm_bus_vectors cam_video_vectors[] = {
	/* Qualcomm's patch for case 01281113 */
	{
		.src = MSM_BUS_MASTER_VFE,
		.dst = MSM_BUS_SLAVE_EBI_CH0,
		.ab  = 800000000,
...
#endif
void mclk_self_adapt_for_driver(void)
{
	char board_name[MAX_SENSOR_NAME] = {0};
	get_camera_sensor_name(board_name, MAX_SENSOR_NAME);
	if (0 == strcmp(board_name, "g527")||0 == strcmp(board_name, "g740"))
		{
		cam_settings[9].drv = GPIOMUX_DRV_6MA;
		}
	if (0 == strcmp(board_name, "g740"))	
		{
		cam_settings[1].drv = GPIOMUX_DRV_4MA;
		}
		
     printk("%s: board_name =%s , cam_settings[9].drv =%d , cam_settings[1].drv=%d\n", __func__, board_name,cam_settings[9].drv,cam_settings[1].drv);
}

void __init msm8930_init_cam(void)
{
	mclk_self_adapt_for_driver(); 
	...
	/* move this sentence up to the beginning of the function */
}
