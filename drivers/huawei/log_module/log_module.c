...
DEVICE_ATTR(set_device_log, 755, NULL,set_device_log_store);
DEVICE_ATTR(get_num, 755, get_num_show, NULL);
DEVICE_ATTR(get_devices, 755, get_devices_show, NULL);

static struct attribute * log_module_dev_attrs[] = {
...
static int log_module_set_device_log(struct hw_device_debug * device_in)
{
    ...
    if(device_local->dynamic_debug_config)
      rc = device_local->dynamic_debug_config(&device_local->log_level);
    /* we just allow one log methed, so we use else if here */
    else if(device_local->debug_mask_config)
        rc = device_local->debug_mask_config(&device_local->log_level);
...
static struct miscdevice log_module_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "log_module_interface",
	.fops = &log_module_fops,
};
...
static int __init hw_debug_log_init_module(void)
{
    ...
    ret = misc_register(&log_module_dev);
...
#ifdef SELF_DEBUG
    ret = sysfs_create_group(&log_module_dev.this_device->kobj, &log_module_dev_attr_grp);
#endif
...
static void __exit hw_debug_log_exit_module(void)
{
    ...
    ret = misc_deregister(&log_module_dev);
}
