...
char * get_synaptics_touch_info(void)
{
	...
	if (IC_TYPE_2202 == syn_version.syn_ic_name)
	{
		...
		snprintf(touch_info, sizeof(touch_info), "synaptics-2202-%s.%d", module_name, config_id);		
	}
	else if (IC_TYPE_3200 == syn_version.syn_ic_name)
	{
		config_id = query_name[3];
		snprintf(touch_info, sizeof(touch_info), "synaptics-3200-%s.%d", module_name, config_id);	
	}
	else
	{
		config_id = query_name[3];
		snprintf(touch_info, sizeof(touch_info), "synaptics-%s.%d", module_name, config_id);	
	}
	return touch_info;
}
...
err_check_functionality_failed:
	if(touch_pdata && touch_pdata->touch_power)
	{
		if(touch_pdata->touch_power(IC_PM_OFF))
...
int RMI4_disable_program(struct i2c_client *client)
{
	...
	if (i2c_smbus_write_byte_data(client,fd_01.commandBase,0x01))
		pr_err("%s,Issue a reset command failed\n",__func__);
...
static int RMI4_enable_program(struct i2c_client *client)
{
	...
	if (i2c_smbus_write_word_data(client,fd_34.dataBase+2,bootloader_id) < 0)//write Block Data 0
		pr_err("%s,write Block Data0 failed\n",__func__);
...
static int RMI4_program_firmware(struct i2c_client *client,const unsigned char *pgm_data)
{
	...
	bootloader_id = i2c_smbus_read_word_data(client,fd_34.queryBase);
	if(i2c_smbus_write_word_data(client,fd_34.dataBase+2, bootloader_id ) < 0) //write Block Data0
		pr_err("%s,write Block Data0 failed\n",__func__);
