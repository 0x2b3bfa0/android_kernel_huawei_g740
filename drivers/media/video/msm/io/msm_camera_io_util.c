	...
	if (gpio_en) {
		...
		if(! sinfo->standby_is_supported || ! sinfo->standby_mode){
			...
			for(i = 0; i < gpio_conf->cam_gpio_req_tbl_size; i++, gpio_conf->cam_gpio_req_tbl++)
			{
				...
				{
					rc = gpio_request_one(gpio_conf->cam_gpio_req_tbl->gpio, 
						gpio_conf->cam_gpio_req_tbl->flags, gpio_conf->cam_gpio_req_tbl->label);
					/*coverity:Calling function "gpio_request_one" without checking return value */
					if(rc < 0)
					{
						pr_err("%s camera pwd gpio = %d request failed\n", __func__,gpio_conf->cam_gpio_req_tbl->gpio);
						return rc;
					}
					break;
				}
