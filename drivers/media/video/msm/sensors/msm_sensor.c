	...
	const struct i2c_device_id *id)
{
	...
	if (s_ctrl->sensordata == NULL) {
        ...
        if(s_ctrl->func_tbl->sensor_customize_sensor_config)
            s_ctrl->func_tbl->sensor_customize_sensor_config(s_ctrl);

        if(s_ctrl->func_tbl->sensor_customize_platform_data)
            s_ctrl->func_tbl->sensor_customize_platform_data(s_ctrl);
