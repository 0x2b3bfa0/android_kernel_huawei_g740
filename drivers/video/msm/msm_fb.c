...
static int msm_fb_set_display_inversion(struct msm_fb_data_type *mfd, unsigned int inversion_mode)
{
	...
	pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
	if ((pdata) && (pdata->set_display_inversion))
	{
		down(&mfd->sem);
		ret = pdata->set_display_inversion(mfd,inversion_mode);
		up(&mfd->sem);
	}
	else
	{
		MSM_FB_INFO("This panel can not support display inversion function");
	}
...
/* Add dynamic gama function */
	...
	case MSMFB_DISPLAY_INVERSION:
		...
		if (ret){
			printk(KERN_ERR "%s:MSMFB_DISPLAY_INVERSION copy from user failed \n", __func__);
			return ret;
		}

		ret = msm_fb_set_display_inversion(mfd, lcd_display_inversion);
		if (ret){
			printk(KERN_ERR "%s:MSMFB_DISPLAY_INVERSION set display_inversion failed = %d \n", __func__, lcd_display_inversion);
		}
		break;
#endif
	default:
