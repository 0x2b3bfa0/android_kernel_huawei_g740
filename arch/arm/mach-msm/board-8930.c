...
static struct touch_settings cyttsp4_sett_ofilm_G718_param_regs = {
	.data = (uint8_t *)&cyttsp4_ofilm_G718_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_G718_param_regs),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_ofilm_G740_L00_param_regs = {
	.data = (uint8_t *)&cyttsp4_ofilm_G740_L00_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_ofilm_G740_L00_param_regs),
	.tag = 0,
};

#include <linux/Config_Truly.h>
static struct touch_settings cyttsp4_sett_truly_G718_regs = {
	.data = (uint8_t *)&cyttsp4_truly_G718_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_truly_G718_param_regs),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_truly_G740_L00_regs = {
	.data = (uint8_t *)&cyttsp4_truly_G740_L00_param_regs[0],
	.size = ARRAY_SIZE(cyttsp4_truly_G740_L00_param_regs),
	.tag = 0,
};

/* add config for ofilm and truly */
struct cyttsp4_sett_param_map cyttsp4_config_param_map[] = {
	[0] = {
			.id = OFILM,
			.product_family = "G718",
			.param = &cyttsp4_sett_ofilm_G718_param_regs,
		},
	
	[1] = {
			.id = TRULY,
			.product_family = "G718",
			.param = &cyttsp4_sett_truly_G718_regs,
		},
	
	[2] = {
			.id = OFILM,
			.product_family = "G740_L00",
			.param = &cyttsp4_sett_ofilm_G740_L00_param_regs,
		},
	[3] = {
			.id = TRULY,
			.product_family = "G740_L00",
			.param = &cyttsp4_sett_truly_G740_L00_regs,
		},
	[4] = {
			.id = LENSONE,
			.product_family = NULL,
		...
		int on, struct device *dev)
{
	...
	if (on) {
		rc = gpio_request(rst_gpio, "TOUCH_RESET");
		if (rc < 0) {
			gpio_free(rst_gpio);
			rc = gpio_request(rst_gpio, "TOUCH_RESET");
		}
		if (rc < 0) {
			...
			if (rc < 0) {
				...
				rc = gpio_request(irq_gpio, "TOUCH_INT");
				if (rc < 0) {
					gpio_free(irq_gpio);
					rc = gpio_request(irq_gpio,"TOUCH_INT");
				}
