...
/*move this place for pablic use*/
...
int lcd_detect_panel(const char *pstring)
{
    ...
    id = get_lcd_id();
    /*coverity:  a buffer overflow */
    snprintf(psKey, MAX_KEY_LENGTH, "lcd/id%X", id);
...
int get_lcd_name(char *pname,int length)
{
    ...
    memset(psKey, 0, MAX_KEY_LENGTH);
    id = get_lcd_id();
    /*coverity:Using "sprintf" can cause a buffer overflow*/
    snprintf(psKey, MAX_KEY_LENGTH, "lcd/id%d", id);
...
/*get whether need RF adjust*/
/*to support a RF adjust feature, only add product name in xml file,such as G718*/
void get_rf_adapt(char *pname,int length)
{
	int ret = 0;
	if((NULL == pname) || (0 >= length))
	{
		printk(KERN_ERR "%s:Invalid input RF parameters!\n",__func__);
		return;
	}

	ret = get_hw_config_string("lcd/rf", pname, length, NULL);
	if(false == ret)
	{
		printk(KERN_ERR"Get RF feature faild!\n");
		strncpy(pname,RF_DEFAULT,MAX_RF_NAME_LEN);
	}
}
...
/*get lcd backlight device type*/
hw_lcd_ctrl_bl_dev  get_lcd_ctrl_bl_dev(void)
{
	hw_lcd_ctrl_bl_dev lcd_ctrl_bl_dev = BY_WLED;
	bool ret = false;
	ret = get_hw_config_enum("lcd/bl_dev", (char*)&lcd_ctrl_bl_dev, sizeof(hw_lcd_ctrl_bl_dev), NULL);
	if(false == ret)
	{
		lcd_ctrl_bl_dev = BY_WLED;
		return lcd_ctrl_bl_dev;
	}
	else
	{
		return lcd_ctrl_bl_dev;
...
/*===========================================================================
FUNCTION     get_wled_string_num

DESCRIPTION
  This function provide the number of wled, as same as the number of LEDs.

DEPENDENCIES

RETURN VALUE
  Number of WLED number

SIDE EFFECTS
  None
===========================================================================*/
#define TWO_STRINGS 2
u8 get_wled_string_num(void)
{
	int wled_string_num = 0;
	bool ret = false;

	ret = get_hw_config_int("lcd/wled_strings_num",  &wled_string_num, NULL);
	if(false == ret)
	{
		printk(KERN_ERR "%s,get lcd wled_strings_num fail!\n",__func__);
		return TWO_STRINGS;
	}
	return (u8)wled_string_num;
}
/*===========================================================================
FUNCTION     wled_whistle_parameter

DESCRIPTION
  This function provide the parameter of WLED_CNTRL_15,to slove whistle problem.

DEPENDENCIES

RETURN VALUE
  Value of WLED_CNTRL_15

SIDE EFFECTS
  None
===========================================================================*/
u8 wled_whistle_parameter(void)
{
	int wled_whistle_para = 0;
	bool ret = false;

	ret = get_hw_config_int("lcd/wled_whistle_parameter", &wled_whistle_para, NULL);
	if(false == ret)
	{
		printk(KERN_ERR "%s,get lcd wled_whistle_para fail!\n",__func__);
		return WLED_WHISTLE_DEFAULT_VALUE;
	}
	return (u8)wled_whistle_para;
}
/*===========================================================================
...
int get_battery_name(int batt_r_id,char *pname,int length)
{
    ...
    memset(psKey, 0, MAX_KEY_LENGTH);
    snprintf (psKey, MAX_KEY_LENGTH, "battery/R_%d", batt_r_id);
    ret = get_hw_config_string(psKey, pname, length, NULL);
    kfree(psKey);
    if(false == ret)
    {
...
tp_test_config get_tp_test_config(tp_test_data **data)
{
    bool ret = false;
    tp_test_config type = TP_CONF_DEFAULT;
    ret = get_hw_config_enum("tp/test_config", (char*)&type, sizeof(type), NULL);
    if (false == ret)
    {
        HW_CONFIG_DEBUG(KERN_ERR "tp test data read fail switching to default type=%d!\n", type);
    }

    HW_CONFIG_DEBUG("tp test config is:%d\n",type);
    *data = &test_data_table[type];

    return type;
}
#define CAPACITANCE_INFO_NUM 9
/*default value,default 5 inch*/
cypress4_tp_cap_info cap_date = 
{
    .tx_lines = 24,/*Tx lines for the panel*/
    .rx_lines = 13,/*Rx lines for the panel*/
    .min_mut_cap = -3300,/*minimum mutual capacitance value*/
    .max_mut_cap = -1200,/*maximum mutual capacitance value*/
    .min_self_cap = -500,/*minimum self capacitance value*/
    .max_self_cap = 1300,/*maximum self capacitance value*/
    .data_start_byte = 8,/*first byte of data after command header*/
    .data_ok = false,
    .ignore_list_size_self_cap = 0,/*no of elements in the self cap ignore list*/
    .ignore_list_size_mut_cap = 0,/*no of elements in the mutual cap ignore list*/
    .ignore_list_self_cap = NULL,/*pointer to the list of self capacitance list*/
    .ignore_list_mut_cap = NULL,/*pointer to the list of mutual capacitance list*/
};
void get_tp_capacitance_info(cypress4_tp_cap_info **date)
{
    char info_string[MAX_DEVICES_LENGTH] ;
    bool ret ;
    int i = 0,value;
    char *p;
    char *buf;
    ...
    *date = &cap_date;
    ret = get_hw_config_string("tp/capacitance_info", info_string, sizeof(info_string), NULL);
    if( true != ret)
    {
        printk("%s,get capacitance_info fail!\n",__func__);
        return;
    }
    HW_CONFIG_DEBUG("hsad: TP capacitance_info. \"%s\". \n", info_string);
    buf = info_string;
    /*The Order is below,and default value is
     *TX  RX  min_mut_cap max_mut_cap min_self_cap max_self_cap
     *24  13    -2950       -1500        -600          1300 
     */
    for(i = 0; i < CAPACITANCE_INFO_NUM ; i++)
    {
        p = strsep(&buf,"_");
        if(!p)
        {
            printk("strsep error is hsad in get_tp_capacitance_info \n");
            return;
        }
        value = atoi_light(p);
        switch(i)
        {
            case 0://TX
                cap_date.tx_lines = value;break;
            case 1://RX
                cap_date.rx_lines = value;break;
            case 2://min_mut_cap
                cap_date.min_mut_cap = value;break;
            case 3://max_mut_cap
                cap_date.max_mut_cap = value;break;
            case 4://min_self_cap
                cap_date.min_self_cap = value;break;
            case 5://min_self_cap
                cap_date.max_self_cap = value;break;
            case 6://ignore_list_size_mut_cap
                cap_date.ignore_list_size_mut_cap = value;break;
            case 7://ignore_list_size_self_cap
                cap_date.ignore_list_size_self_cap = value;break;
            case 8://data_start_byte
                cap_date.data_start_byte = value;break;
            default:
                printk("%s,Transform error\n",__func__);break;
        }
    }
    cap_date.data_ok = true;
}

void get_tp_capacitance_ignore_list(int* data, char* name, int* count)
{
    bool ret = false;
    char config_key_name[MAX_KEY_LENGTH] = {'\0'};
    char info_string[MAX_DEVICES_LENGTH] = {'\0'};
    int i = 0,value = 0;
    char *p = NULL;
    char *buf = NULL;

    if (NULL == data || NULL == name || NULL == count)
    {
        *count = 0;
    ...
    snprintf(config_key_name, sizeof(config_key_name), "tp/%s_ignore_list", name);

    ret = get_hw_config_string(config_key_name, info_string, sizeof(info_string), NULL);
    if (false == ret)
    {
        HW_CONFIG_DEBUG(KERN_ERR "get brightness value fail for %s!\n", name);
        *count = 0;
    }
    ...
    for (i = 0; i < *count ; i++)
    {
        ...
        if(!p)
        {
            pr_err("The list count and list are not in sync!\n");
            *count = 0;
            return;
        }
        value = atoi_light(p);
...
void get_light_value(int *lux)
{
    ...
    {
        p = strsep(&buf,"_");
        /*coverity:Event var_compare_op:  Comparing "p" to null implies that "p" might be null. */
        if(!p)
        {
...
hw_sensor_prop get_sensor_prop(void)
{
    hw_sensor_prop sensor_prop = SENSOR_PROP_NONE;
    bool ret = false;
    ret = get_hw_config_enum("sensors/prop", (char*)&sensor_prop, sizeof(sensor_prop), NULL);
    if(false == ret)
        sensor_prop = SENSOR_PROP_NONE;
	return sensor_prop;
}
...
pmic_gpio_type get_pmic_gpio_type(int gpio)
{
    ...
    memset(psKey, 0, MAX_KEY_LENGTH);
    /*coverity:Using "sprintf" can cause a buffer overflow*/
    snprintf(psKey, MAX_KEY_LENGTH,"pmicgpio/%d", gpio);
...
int get_led_brightness(const char* name, char* type)
{
    int cur = -1;
    bool ret = false;
    char config_key_name[MAX_KEY_LENGTH] = {'\0'};
    ...
    /*
    keeping the function generic to get different kind of brightness value for led
    if the key is not found it will safely return error, in case of NULL string it will fail
    as string not found so the check is not done
    */
    snprintf(config_key_name, sizeof(config_key_name), "led/%s_%s_brightness", name, type);

    ret = get_hw_config_int(config_key_name, &cur, NULL);
    if (false == ret)
    {
        HW_CONFIG_DEBUG(KERN_ERR "get brightness value fail for %s of type:%s !\n", name, type);
        return -1;
    }

    HW_CONFIG_DEBUG("LED %s brightness of type:%s is %d\n", name, type, cur);
    if ( cur < 0 || cur > 255 ) {
        HW_CONFIG_DEBUG("use default brightness for %s of type %s\n", name, type);
        cur = -1;
    }
